...
def generate_stirap_sequence(voltage_data, time_data, down_leg_v):
    ...
    sequence_up[n_on + n_hold + n_delay : n_on + n_hold + n_stirap + n_delay] = (
        np.linspace(0.0, v_up, n_stirap)
    )
    sequence_up[n_on + n_hold + n_stirap + n_delay : n_sequence] = np.linspace(
        v_up, v_up, n_off
    )
    ...
    sequence_down[n_on + n_hold : n_on + n_hold + n_stirap] = np.linspace(
        v_down, 0.0, n_stirap
    )

    # Mirror the sequences to generate dissociation pulse
    sequence_up[int(N / 4) : int(N / 4) + n_sequence] = np.flipud(
        sequence_up[0:n_sequence]
    )
    sequence_down[int(N / 4) : int(N / 4) + n_sequence] = np.flipud(
        sequence_down[0:n_sequence]
    )
...
def write_stirap_to_fg(gpib_addr, seq, sysmsg):
    ...
    for k in gpib_addr:
        ...
        if device == 0:  # down
            address = "USB0::0x0957::0x0407::MY44037562::INSTR"
        elif device == 1:  # up
            address = "USB0::0x0957::0x0407::MY43000220::INSTR"

        if address in resources:
            sysmsg.setText(
                "Communicating with device {0:d} at address: {1}.".format(
                    device, address
                )
            )
            sleep(1.0)
            ...
            instr_id = instr.query("*IDN?")
            sysmsg.append(instr_id)
